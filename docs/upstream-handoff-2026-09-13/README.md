...
├── 模拟数据生成逻辑.md               模拟数据生成算法与分布验证（答辩材料）
...
├── real_dist.json                 真实数据分布模型（生成器学习依据，勿删）
├── simdata\                       预生成模拟 CSV（5000订单，分布与真实一致，可直接上传）
│   ├── nvv2t.csv / dsv13r2.csv / nvv2t_md_end.csv
...
```bash
# 模拟 CSV 已预生成在交付包 simdata/，直接上传即可（或自行重新生成）：
#   python 02_gen_aug_sim_data.py --orders 5000 --out <目录>
cd /home/hadoop
hive -f 01_sim_create.hql        # 建 ncs_sim_* 四库
hive -f 03_sim_load.hql          # 导入模拟 CSV（先 scp simdata/*.csv 到 /home/hadoop/simdata/）
...
3. 正式库行数：ODS/DWD 均 3395 订单 / 1594 监测 / 105 站点；模拟库（v2 分布一致版）5000 订单 / ~2292 监测 / 105 站点
