...
| 接口域 | 提供方 | 调用方 | 前缀 | 网络范围 |
| --- | --- | --- | --- | --- |
| 大屏查询 API | `analytics-query-api` | Vue/ECharts 投屏页面 | `/api/v1` | 按部署需要开放 |
| 数据管理 API | `data-admin-service` | 调度器、内部管理工具、`ml-pipeline` | `/internal/v1` | 仅内网 |
| 机器学习作业 API | `ml-pipeline` | 调度器、内部管理工具 | `/ml-internal/v1` | 仅内网 |
