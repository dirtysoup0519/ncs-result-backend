...
> 状态：公共抽象合同；大屏主要查询路由已实现，ADS 包导入已按版本化合同支持 v2.1/v2.3/v2.5，预测管理 API（/internal/v1/ml/*）已实现，结果表内部查询接口（/internal/v1/ads-result/*）已实现
> 契约版本：`1.0-rc1`
...
> 当前业务与部署结构以 `项目业务架构与代码规划.md` 为准；模型训练接口仍不提供，模型登记、激活和受控预测运行接口已在内部管理面实现。

> **当前范围覆盖说明（2026-09-15）**：训练、调参和评估接口不在范围内；`/internal/v1/ml/*` 仅用于服务器本地模型登记、激活和推理运行，不对前端开放。大屏预测查询只有在存在已发布预测批次时才启用。详见 `当前范围决策.md`。
