...
包解析已改为版本化合同：`specs_for_schema_version` 按 Manifest `schemaVersion` 选择数据集集合（2.0/2.1 → 10 个，2.2+ → 18 个），相同数据集沿用相同字段合同，新数据集由 v2.5 独立 Schema 描述；未通过"忽略未知文件"绕过合同校验。
...
`admin/prediction_schema.py` 提供 `ctl_model_version`、`ctl_prediction_run`、`rpt_load_prediction` 表和 `api_v1_load_prediction` 视图（MySQL 索引使用方言变体）；模型适配器、负荷数据集构建器、原子发布、预测管理 API（`/internal/v1/ml/*`）和 `scripts/run_load_prediction.py` 入口均已接入，测试通过（张量构建测试在缺 numpy/torch 环境自动跳过）。
...
|---:|---|---|
| 0 | 成功，或没有新批次 | 否 |
| 2 | 环境配置错误：`NCS_ADS_EXCHANGE_ROOT` 不是绝对路径 | 否 |
| 10 | 锁被其他同步进程占用 | 否 |
