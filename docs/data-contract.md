...
> 适用范围：Hive ADS 交付、MySQL 结果库、数据库管理服务、查询后端、Web 大屏和受控模型推理结果；模型训练章节仅为外部团队参考
> 契约版本：`1.0-draft`

> **当前范围覆盖说明（2026-09-15）**：本项目不训练、不调参、不评估模型；但负责接收外部模型结构和权重，基于已发布 `load_hourly` 执行受控推理，并按批次、质量和发布合同写入预测结果。详见 `当前范围决策.md` 与 `ADS_v2.5自动同步与模型推理设计.md`。
