...
> 状态：公共抽象契约；通用批次/发布合同已落地，ADS 包合同已版本化（v2.1 → 10 数据集，v2.5 → 18 数据集），导入与发布均通过验收工具验证
> 适用范围：Hive ADS 交付、MySQL 结果库、数据库管理服务、查询后端和 Web 大屏；机器学习章节仅为未来参考
> 契约版本：`1.0-draft`

> **当前范围覆盖说明（2026-09-14）**：机器学习特征、训练、模型登记和预测运行合同不属于当前实现范围，相关章节仅供未来参考。当前只校验和管理上游交付的处理后数据；若交付预测结果，其中的模型字段只作为上游血缘原样保存。详见 `当前范围决策.md`。

## 1. 契约目标

本文件定义各模块交换数据时必须共同遵守的语义，不规定 Hive SQL、MySQL DDL、Python 类或具体机器学习算法。

最新 v2.3 交付采用 Schema/metric version `2.0.0`、`FULL_SNAPSHOT` 和 `SUCCESS` Manifest，共 10 个 CSV。新增完整 `station_daily`、`station_reference`、带日期的 `station_hour_daily` 及含质量标记的连续 `load_hourly`。详细字段、粒度和核验结论见 `ADS_Spark_v2.3交接包评审.md`；在 v2.3 适配器完成前，不得把这些字段描述为已发布 API 能力。
...
| 未来模型交付 | 已处理预测结果 | 模型团队输出 | 当前不训练；以后按普通结果数据集接入 |
...
| Vue 3 / DataV 大屏 | 可视化组件 | 查询 API | 展示数据，不重新定义统计口径 |
