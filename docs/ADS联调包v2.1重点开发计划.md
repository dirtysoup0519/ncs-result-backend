...
该包已经满足启动 ADS 联调开发的最低条件，可以开始实现“联调包校验 → staging → 结果表 → 发布 → 只读视图 → 查询 API”的代码。它仍是 `ncs_sim_dashboard` 模拟联调基线，不是最终生产合同，因此本轮目标是完成可替换的数据适配器和端到端联调，不把其中的物理表名、全历史粒度或模拟数值固化为最终业务语义。
...
- ADS-5 已完成联调准备：新增 MySQL 元信息/视图合同验证脚本、环境测试入口和账号权限边界文档；真实 MySQL 迁移、导入、回滚和权限验证仍等待环境凭据。
- 本轮使用 SQLite 与真实 v2.1 包完成冒烟验证；没有把 SQLite 结果计入 MySQL 验收。
