...
`gen_sim_data.py`：参数化模拟生成器，**表头与真实数据完全一致（22/11/8 列、UTF-8 BOM）**，特征对齐真实数据（纯数字 ID 郑州站点、异常年份比例可配、零费用比例可配、负电流 -22~-74A、温度 31~35℃、科学计数法时间戳、esd 100% 关联订单）。

```cmd
python gen_sim_data.py --orders 5000 --year 2019 --abnormal 1.0 --zero-fee 0.5 --stations 105 --out 输出目录
```

常用参数：--orders 订单数 / --year 年份 / --abnormal 异常年份比例 / --zero-fee 零费用比例 / --stations 站点数 / --seed 随机种子
...
| gen_sim_data.py | 模拟数据生成器（贴合真实特征） |
