...
> 当前接入支持 Windows 远程输入和虚拟机就地处理两种目标模式，统一方案见 `项目业务架构与代码规划.md`。

> 对接对象：ADS 负责人、ODS/DWD 负责人、结果库与后端负责人；模型负责人仅确认数据口径
>
> 当前范围：处理后数据接入、结果库存储、大屏查询后端，以及使用外部模型和权重执行受控推理；不包含模型训练、调参和效果评估
> 事实来源：最新版 `交接包(2).zip` 中 ODS/DWD/DWS/ADS 脚本、数据交接说明，以及模型同学提供的测试模型交接信息
>
> 实施状态：ADS v2.1 已完成 MySQL 首轮接入；ADS Spark v2.3 已核验、尚待代码适配。本文未关闭事项面向 v2.3 正式化/生产合同。

## 1. 对接结论

ADS v2.3 已补齐完整站点日报、站点维表、日期化热力图、平台订单占比和连续小时质量字段，但仍不能视为正式生产合同。下一轮对接重点不是让 ADS 提供更多脚本，而是冻结以下四类合同：
